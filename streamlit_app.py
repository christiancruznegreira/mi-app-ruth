...
with col2:
    if st.button("⚖️ Análisis"):
        procesar_prompt(f"Haz un análisis experto sobre un tema legal...", modo)
        st.rerun()
with col3:
    if st.button("📦 Optimización"):
        procesar_prompt(f"Como experta en Amazon, optimiza un listado...", modo)
        st.rerun()
with col4:
    if st.button("💡 Estrategia"):
        procesar_prompt(f"Propón una idea de negocio disruptiva...", modo)
        st.rerun()
