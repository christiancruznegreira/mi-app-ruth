...
# --- 1. ESTÉTICA ANDROID CRYSTAL (CSS DE ALTA PRIORIDAD) ---
...
st.markdown("""
    ...
    /* FLECHA DE RESCATE ROJA (FIJA PARA ANDROID) */
    [data-testid="stSidebarCollapsedControl"], button[kind="headerNoContext"] {
        background-color: #ff4b4b !important;
        color: white !important;
        border-radius: 0 20px 20px 0 !important;
        left: 0 !important; top: 15px !important;
        width: 65px !important; height: 55px !important;
        display: flex !important; align-items: center !important; 
        justify-content: center !important; z-index: 9999999 !important;
        box-shadow: 5px 0 20px rgba(255, 0, 0, 0.5) !important;
    }
    [data-testid="stSidebarCollapsedControl"] svg { fill: white !important; width: 30px; height: 30px; }

    /* BARRA LATERAL CRISTAL */
    ...
    /* TÍTULO NEÓN AJUSTADO PARA MÓVIL */
    @keyframes flicker {
        0%, 18%, 22%, 25%, 53%, 57%, 100% { text-shadow: 0 0 4px #f00, 0 0 11px #f00, 0 0 19px #f00; color: #ff4b4b; }
    ...
    .ruth-subtitle { text-align: center; color: #666; font-size: 0.6rem; letter-spacing: 0.3rem; margin-top: -10px; margin-bottom: 2rem; font-weight: bold;}

    /* BOTONES GHOST (TEXTO PURO) */
    .stButton>button { 
        border: none !important; background-color: transparent !important; color: #888 !important; 
        width: 100% !important; text-transform: uppercase; font-size: 0.5rem !important; 
        letter-spacing: 0.1rem; transition: 0.3s;
    }
    .stButton>button:hover { color: #ff4b4b !important; text-shadow: 0 0 10px #f00; }

    /* INPUTS LOGIN (SIN ETIQUETAS) */
...
# --- 3. GESTIÓN DE ACCESO (LOGIN/REGISTRO MINIMALISTA) ---
...
def login_ui():
    ...
    with col_c:
        if st.session_state.auth_mode == "login":
            ...
            if st.button("REGISTRAR NUEVA CUENTA"):
...
with st.sidebar:
    ...
    esp_act = st.selectbox("E:", list(ESPECIALIDADES.keys()))
    ton_act = st.selectbox("T:", list(TONOS.keys()))
