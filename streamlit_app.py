...
# --- 1. ESTÉTICA PREMIUM Y TÍTULO NEÓN ---
st.set_page_config(
...
try:
    ...
    st.error("Error en conexión. Revisa tus Secrets.")
    st.stop()

# --- 3. EXPERTOS ---
EXPERTOS = {
    "Abogada": "Eres RUTH Abogada Senior. Rigor legal, sin disculpas.",
    "Amazon Pro": "Eres RUTH Especialista Amazon. SEO y ventas. Directa.",
...
# --- 5. CUERPO PRINCIPAL (LOS 4 BOTONES RECUPERADOS) ---
...
with col1: 
    if st.button("📝 Email"): enviar_rapido(f"Redacta un correo profesional como {modo}."); st.rerun()
with col2: 
    if st.button("⚖️ Análisis"): enviar_rapido(f"Haz un análisis experto como {modo}."); st.rerun()
with col3: 
    if st.button("📦 Amazon"): enviar_rapido(f"Optimiza este caso como experta en Amazon."); st.rerun()
with col4: 
    if st.button("💡 Idea"): enviar_rapido(f"Propón una idea disruptiva como {modo}."); st.rerun()
