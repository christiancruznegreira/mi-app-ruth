...
with col1:
    if st.button("📝 Email"):
        procesar_prompt(f"Como {modo}, redacta un correo profesional...", modo)
        st.rerun()

with col2:
    if st.button("⚖️ Análisis"):
        procesar_prompt(f"Haz un análisis experto sobre...", modo)
        st.rerun()

with col3:
    if st.button("📦 Optimización"):
        procesar_prompt(f"Como experta en Amazon, optimiza...", modo)
        st.rerun()

with col4:
    if st.button("💡 Estrategia"):
        procesar_prompt(f"Propón una idea de negocio desde tu visión de {modo}...", modo)
        st.rerun()
...
for msg in st.session_state.messages:
    avatar = ruth_avatar if msg["role"] == "assistant" else None
